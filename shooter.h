...
public:
    ...
    {
        // shoot a bullet
        // returns a default bullet located at head position of shooter
    ...
    {   
        ...
        if (is_moving_left)
        {
            vel = -vel;     // set direction of velocity
        }

        double new_body_x = body.getX() + vel*t;    // new x coordinate of body
        double new_head_x = head.getX() + vel*t;    // new x coordinate of head
        ...
        body.moveTo(new_body_x, body.getY());   // move body shape at new coordinates
        head.moveTo(new_head_x, head.getY());   // move head shape at new coordinates
    }
    ...
    void set_color(Color shooter_color)
    {
        // set color of shooter
        head.setColor(shooter_color);
        body.setColor(shooter_color);
    }    
